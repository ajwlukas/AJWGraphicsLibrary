...
ASELexer::ASELexer(LPSTR p_FileName, DWORD p_SizeOfBuffer) : CLexer(p_FileName, p_SizeOfBuffer)
{
}
...
//Lexical Analysis
LONG ASELexer::GetToken(LPSTR p_TokenString)
{
	// 1) 앞쪽 쓸데없는 문자찾기
	...
	else if (Char == '*')
	{
		...
		{
			if (!strcmp(p_TokenString, Token[i].c_str()))
			{
